...
scripts/
├── sync_all.rb             # Master orchestrator for all sync scripts
├── sync_events.rb          # Sync events from PC Calendar
├── sync_groups.rb          # Sync groups with leaders
├── sync_hero_images.rb     # Sync hero slider images from PCO
├── sync_facebook_photos.rb # AI-powered photo selection from Facebook
├── sync_team.rb            # Sync team member profiles
├── sync_cloudflare_video.rb # Sync latest video from Cloudflare Stream
├── pco_client.rb           # Planning Center API client (singleton)
└── image_utils.rb          # Image optimization utilities
